document.addEventListener("DOMContentLoaded", () => {
  ...
  const menuCleanTasks = document.querySelector(".menu-clean-tasks");
  ...
  function initPomodoros() {
    ...
    function startCountDown(e) {
      ...
      if (!timerElements.isRunning && timerElements.totalSeconds === 0) {
        const [minutes, seconds] = currentTimer.innerText
          .split(":")
          .map(Number);
  ...
  function initTasks() {
    ...
    function addDropDownEvent(btnDropDown, index, para, div, span, wrapper) {
      ...
      btnDropDown.addEventListener("click", (e) => {
        ...
        if (divDropDown) {
          ...
          const [remove, divRemove] = createElementRemove(
            index,
            newDivDropDown
          );
    ...
    function createElementCheckBox(index, para) {
      ...
      checkbox.addEventListener("click", () => {
        ...
        tasks[index].done = checkbox.classList.contains(
    ...
    function removeAllCompletedTasks() {
      const removeCompletedTasks = document.querySelector(
        ".menu-clean-tasks [data-completed]"
      );
    ...
    function handleRemoveAllTasks() {
      const removeAllTasks = document.querySelector(".menu-clean-tasks [data-remove-all]");
